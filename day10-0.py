...
def trailhead_score(trail_map, trail_ends, row, col):
    rows, cols = len(trail_map), len(trail_map[0])

    if trail_map[row][col] == 9 and (row, col) not in trail_ends:
    ...
    for dr, dc in directions:
        ...
        if 0 <= new_row < rows and 0 <= new_col < cols and trail_map[new_row][new_col] == 1 + trail_map[row][col]:
            score += trailhead_score(trail_map, trail_ends, new_row, new_col)
    return score
...
for i in range(rows):
    for j in range(cols):
        ...
        trail_ends = set()
        total_score += trailhead_score(trail_map, trail_ends, i, j)
